...
int main(int argc, char **argv) 
{
    ...
    Noise wheel_encoder_dx_noise = Noise(0.0, 0.01);
    Noise wheel_encoder_dy_noise = Noise(0.0, 0.05);
    WheelEncoder wheel_encoder = WheelEncoder();
    Vec3f frame_world = Vec3f(0, 0, 0);
    ...
    {   
        ...
        // while (!waypoints.empty())  // While there's a waypoint to visit
        {
            ...
            // {
                ...
                Vec2f previous_W = Vec2f(robot_ideal.position_W.x(), robot_ideal.position_W.y());
                cout << "prev_W: (" << previous_W.x() << ", " << previous_W.y() << ")" << endl;

                float dx_ns = wheel_encoder_dx_noise.gaussian();
                float dy_ns = wheel_encoder_dy_noise.gaussian();
                wheel_encoder.simulate_odometry( robot_ideal.speed, delta_t, 
                                dx_ns, dy_ns );

                Vec2f next_R = Vec2f(wheel_encoder.dx, wheel_encoder.dy);
                cout << "next_R: (" << next_R.x() << ", " << next_R.y() << ")" << endl;

                Mat3f homo_WR = get_homogeneous_transform_frame_A_to_B(
                                frame_world.x(), frame_world.y(), frame_world.z(), 
                                robot_ideal.frame_robot.x(), robot_ideal.frame_robot.y(), robot_ideal.frame_robot.z());

                Vec3f next_W = homo_WR * next_R.homogeneous();
                ...
                cout << "W.x:" << frame_world.x() << "\tW.y:" << frame_world.y() << "\tW.th:" << frame_world.z() << endl;
                cout << "R.x:" << robot_ideal.frame_robot.x() << "\tR.y:" << robot_ideal.frame_robot.y() << "\tR.th:" << robot_ideal.frame_robot.z() << endl;
