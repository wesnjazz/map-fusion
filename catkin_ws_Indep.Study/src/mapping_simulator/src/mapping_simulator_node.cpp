            ...
            while (true)
            {
                ...
                if (if_arrived_at_xy_frameW(robot_actual, arrival_W.x(), arrival_W.y(), 0.2)) {
                    cout 
                        ...
                        << "\t\t Robot speedhhh: " << robot_actual.speed 
                ...
                Vec3f pivot_R_actual = Vec3f(0, 0, 1);   /** current position in the current frame (excluding angle) **/
                Vec3f next_frame_in_Wframe_actual = accumulated_frames_HT_next_frame_in_Wframe_actual * pivot_R_actual;  // Pure Translation
                Vec2f next_position_in_Wframe_actual = Vec2f(next_frame_in_Wframe_actual.x(), next_frame_in_Wframe_actual.y());    // cut unnecessary third element
                ...
                Vec3f pivot_R_ideal = Vec3f(0, 0, 1);
                Vec3f next_frame_in_Wframe_ideal = accumulated_frames_HT_next_frame_in_Wframe_ideal * pivot_R_ideal;
                Vec2f next_position_in_Wframe_ideal = Vec2f(next_frame_in_Wframe_ideal.x(), next_frame_in_Wframe_ideal.y());
                ...
                /** Robot_actual **/
                Mat3f current_frame_HT_next_frame_in_Wframe_actual = current_frame_HT_next_frame_in_Wframe_actual.setIdentity();
                current_frame_HT_next_frame_in_Wframe_actual.block<2, 2>(0, 0) = current_heading_ROT_adjusted_heading_actual;
                current_frame_HT_next_frame_in_Wframe_actual.block<3, 1>(0, 2) = Vec3f(0, 0, 1);
                /** Robot_ideal **/
                Mat3f current_frame_HT_next_frame_in_Wframe_ideal = current_frame_HT_next_frame_in_Wframe_ideal.setIdentity();
                current_frame_HT_next_frame_in_Wframe_ideal.block<2, 2>(0, 0) = current_heading_ROT_adjusted_heading_ideal;
                current_frame_HT_next_frame_in_Wframe_ideal.block<3, 1>(0, 2) = Vec3f(0, 0, 1);


                // // Vec2f aTRANSb_actual = Vec2f(0, 0);
                // // Mat3f current_HT_adjust_heading_actual = current_HT_adjust_heading_actual.setIdentity();
                // // current_HT_adjust_heading_actual.block<2, 2>(0, 0) = current_heading_ROT_adjusted_heading_actual;
                // // current_HT_adjust_heading_actual.block<2, 1>(0, 2) = aTRANSb_actual;

                // // float adjust_angle_goal_robot_frame_radian_ideal = degree_to_radian(angle_degree_to_next_point_in_Robot_frame_ideal / 4.0);
                // // float cos_adjusted_heading_ideal = cut_redundant_epsilon( cos(adjust_angle_goal_robot_frame_radian_ideal) );
                // // float sin_adjusted_heading_ideal = cut_redundant_epsilon( sin(adjust_angle_goal_robot_frame_radian_ideal) );
                // // Mat2f current_heading_ROT_adjusted_heading_ideal;
                // // current_heading_ROT_adjusted_heading_ideal << cos_adjusted_heading_ideal, -sin_adjusted_heading_ideal, sin_adjusted_heading_ideal, cos_adjusted_heading_ideal;
                // // Vec2f aTRANSb_ideal = Vec2f(0, 0);
                // // Mat3f current_HT_adjust_heading_ideal = current_HT_adjust_heading_ideal.setIdentity();
                // // current_HT_adjust_heading_ideal.block<2, 2>(0, 0) = current_heading_ROT_adjusted_heading_ideal;
                ...
                //  *  sHTt = sHT1 * 1HT2 * ... * (k-1)HTk * kHTt
                //  *  **/
                accumulated_frames_HT_next_frame_in_Wframe_actual = HTs_actual.back() * current_frame_HT_next_frame_in_Wframe_actual;
                HTs_actual.pop_back();
                HTs_actual.push_back(accumulated_frames_HT_next_frame_in_Wframe_actual);

                accumulated_frames_HT_next_frame_in_Wframe_ideal = HTs_ideal.back() * current_frame_HT_next_frame_in_Wframe_ideal;
                HTs_ideal.pop_back();
                HTs_ideal.push_back(accumulated_frames_HT_next_frame_in_Wframe_ideal);
